...
module.exports = ({ db, saveProduct, saveMaterial, createCustomer, HttpError }) => kind => context => {
    ...
    rows.forEach((row, index) => {
      ...
      try {
        ...
        if (kind === 'products') {
          ...
          const types = { 'attar stock': 'Attar stock', 'raw material': 'Raw material', packaging: 'Packaging' };
          const typeKey = String(body.type || '').trim().toLowerCase();
          body.type = Object.hasOwn(types, typeKey) ? types[typeKey] : null;
          if (!body.type) throw new Error('Type must be Attar stock, Raw material or Packaging');
          body.unit = String(body.unit || '').trim().toLowerCase();
          if (!['ml', 'pcs'].includes(body.unit)) throw new Error('Unit must be ml or pcs');
          if (body.alertMl === undefined || body.alertMl === null || String(body.alertMl).trim() === '') {
            body.alertMl = body.unit === 'pcs' ? 5 : 50;
          } else {
            if (typeof body.alertMl === 'boolean' || !Number.isFinite(Number(body.alertMl)) || Number(body.alertMl) < 0) throw new Error('Minimum stock (alertMl) must be a number of 0 or more');
            body.alertMl = Number(body.alertMl);
          }
          saveMaterial({ ...context, body, params: {} });
        } else {
