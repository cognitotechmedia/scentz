...
(() => {
  const definitions = {
    ...
    materials: { anchor: '#addProductButton', role: 'role-admin', label: 'Import stock items', title: 'stock items', required: ['name', 'type', 'unit'], columns: ['name', 'type', 'unit', 'code', 'ean', 'alertMl'], sample: ['Example fragrance oil', 'Raw material', 'ml', '', '', 50], help: 'Name, type and unit are required. Type: Attar stock, Raw material or Packaging. Unit: ml or pcs. Optional code and ean identify the item; format barcodes as text in Excel. alertMl is the minimum stock in the chosen unit (blank defaults to 50 ml or 5 pcs). Stock items are available for purchases and production; they are not added to the billing catalogue. Enter quantities and costs separately through Opening stock.' },
  ...
  Object.entries(definitions).forEach(([key, definition]) => {
    const button = document.createElement('button'); button.type = 'button'; button.className = 'outline-button ' + definition.role; button.textContent = definition.label || (key === 'products' ? 'Import billing products' : 'Bulk upload');
    $(definition.anchor).insertAdjacentElement('afterend', button);
    button.onclick = () => { if (busy) return; kind = key; rows = []; outlet = state.outlet.id; user = state.me.id; el('bulkTitle').textContent = `Import ${definition.title || key} · ${state.outlet.name}`; el('bulkHelp').textContent = definition.help; el('bulkResults').textContent = ''; el('bulkFile').value = ''; el('bulkSave').disabled = true; modal.classList.remove('hidden'); };
  });
  ...
  el('bulkFile').onchange = async () => {
    ...
    try {
      ...
      const required = definitions[kind].required || (kind === 'products' ? ['name', 'price', 'gstRate'] : ['name', 'phone']);
  ...
  el('bulkSave').onclick = async () => {
    ...
    try {
      ...
      rows = []; el('bulkResults').textContent = `${result.imported} ${definitions[kind].title || kind} imported successfully.`;
      await reloadAll();
