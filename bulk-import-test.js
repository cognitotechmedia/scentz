...
async function run() {
  ...
  const material = { name: 'Bulk fragrance oil', type: 'Raw material', unit: 'ml', code: 'TEST-OIL', ean: '001234567890', alertMl: '12.5' };
  const beforeStock = await count();
  check('stock preview valid', !(await send('materials', [material], true)).json.errors.length);
  check('stock preview leaves no records', JSON.stringify(await count()) === JSON.stringify(beforeStock));
  for (const invalid of [{ type: 'Wrong' }, { unit: 'kg' }, { alertMl: '-1' }, { alertMl: 'NaN' }, { alertMl: true }, { code: 'bad code' }, { ean: 'x' }]) {
    const result = await send('materials', [material, { ...material, name: 'Invalid stock', code: 'OTHER', ean: '009999999999', ...invalid }]);
    check('invalid stock batch rolls back ' + JSON.stringify(invalid), result.json.imported === 0 && JSON.stringify(await count()) === JSON.stringify(beforeStock));
  }
  check('duplicate stock batch rolls back', (await send('materials', [material, material])).json.imported === 0 && JSON.stringify(await count()) === JSON.stringify(beforeStock));
  const rows = [material, { name: 'Bulk bottle', type: 'Packaging', unit: 'pcs', alertMl: '' }, { name: 'Bulk attar', type: 'Attar stock', unit: 'ml', alertMl: 0 }];
  check('stock import saves all kinds', (await send('materials', rows)).json.imported === 3);
  const imported = (await admin('GET', '/api/state')).json;
  check('stock import creates no billing products', imported.products.length === beforeStock[0]);
  const oil = imported.materials.find(m => m.name === material.name);
  check('stock fields preserved', oil && oil.code === material.code && oil.ean === material.ean && oil.unit === 'ml' && oil.alertMl === 12.5 && oil.stock === 0);
  check('packaging default minimum', imported.materials.find(m => m.name === 'Bulk bottle').alertMl === 5);
  check('explicit zero minimum', imported.materials.find(m => m.name === 'Bulk attar').alertMl === 0);
  check('repeated stock import rejected', (await send('materials', rows)).json.imported === 0);
  check('duplicate stock code rejected', (await send('materials', [{ ...material, name: 'Another oil', ean: '' }])).json.imported === 0);
  check('duplicate barcode rejected', (await send('materials', [{ ...material, name: 'Another oil', code: '' }])).json.imported === 0);
  ...
  for (const kind of ['products', 'customers', 'materials']) check('biller denied ' + kind, (await biller('POST', '/api/import/' + kind, { rows: [customer], preview: true })).status === 403);
}
