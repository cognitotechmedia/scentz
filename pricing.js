...
function costInfo(product) {
  if (product.itemType === 'material') return product.costPerMl > 0 ? { value: product.costPerMl, note: `stock average cost per ${product.unit}` } : { value: null, note: 'stock cost not set' };
...
function loadPriceProduct(product) {
  ...
  hideSuggest();
  const cost = costWithGst(product), stock = product.itemType === 'material' ? product : product.stockMaterialId ? findMaterial(product.stockMaterialId) : null;
...
const priceItems = () => [...products, ...rawMaterials.map(material => ({ ...material, itemType: 'material' }))];
const priceLookup = { choose: (row, item) => loadPriceProduct(priceItems().find(product => product.id === item.id)) };
const priceLookupPool = () => priceItems().map(product => ({ id: product.id, code: product.code || '', ean: product.ean || null, name: product.name, type: product.itemType === 'material' ? `Stock item · ${product.type}` : product.type, stock: product.stock ?? null, unit: product.unit || 'pcs' }));
...
$('#priceForm').addEventListener('submit', event => {
  ...
  submitting($('#savePrice'), async () => {
    ...
    for (const [key, label] of SLABS) {
      ...
      if (!(values[key] > 0) && !(product.itemType === 'material' && key === 'price' && values[key] === 0)) { showToast(`Enter a ${label.toLowerCase()} price greater than 0`); $(NEW_BOXES[key]).focus(); return; }
    }
    ...
    await api('PUT', '/api/prices', { prices: [{ id: product.id, itemType: product.itemType, gstRate: product.gstRate, price: values.price, wholesalePrice: values.wholesalePrice, franchisePrice: values.franchisePrice }] });
...
function renderPrices() {
  ...
  $('#pricesSubtitle').textContent = `Find a billing catalogue product or stock item by code or name, enter its new retail, wholesale and franchise prices (${inclusive ? 'including' : 'before'} GST) and press Enter to save. Changes reach every outlet at once; past bills keep the prices they were made at.`;
  ...
  if (priceProduct) { const fresh = priceItems().find(product => product.id === priceProduct.id); if (!fresh) clearPriceForm(); else priceProduct = fresh; }
  ...
  $('#priceRows').innerHTML = priceItems().map(product => { const cost = costWithGst(product); return `<div class="plist-row" data-load-price="${product.id}" tabindex="0" role="button" aria-label="Load ${escapeHtml(product.name)}"><span><strong>${escapeHtml(product.name)}</strong><small>${escapeHtml(product.code || '')} · ${product.itemType === 'material' ? 'Stock item · ' : ''}${escapeHtml(product.type)} · GST ${product.gstRate}%</small></span><span class="num" data-label="Cost incl. GST">${cost ? currency(cost) : '—'}</span><span class="num" data-label="Retail">${product.itemType === 'material' && !product.price ? '—' : currency(product.price)}</span><span class="num" data-label="Wholesale">${product.wholesalePrice > 0 ? currency(product.wholesalePrice) : 'Set price'}</span><span class="num" data-label="Franchise">${product.franchisePrice > 0 ? currency(product.franchisePrice) : 'Set price'}</span></div>`; }).join('') || '<div class="purchase-empty">No products yet.</div>';
}
const pickPriceRow = event => {
  ...
  event.preventDefault();
  loadPriceProduct(priceItems().find(product => product.id === row.dataset.loadPrice));
