...
const reservedMl = materialId => db.cart.reduce((sum, item) => sum + (item.recipe || []).filter(line => line.id === materialId).reduce((total, line) => total + line.ml, 0) + (item.stockMaterialId === materialId ? item.quantity : 0) + (item.packMaterialId === materialId ? (item.packQty || 1) * item.quantity : 0) + (item.extras || []).filter(extra => extra.id === materialId).reduce((total, extra) => total + extra.qty, 0), 0);
...
function renderCart() {
  if (!db.cart.length) {
    ...
    cartList.innerHTML = db.cart.map(item => `<div class="cart-item"><div>${productVisual(item, true)}</div><div><div class="cart-name">${escapeHtml(item.name)}</div><div class="cart-meta">${item.type}</div><div class="cart-controls"><button class="qty-button" data-qty="${item.id}" data-change="-1">−</button><span>${item.quantity}</span><button class="qty-button" data-qty="${item.id}" data-change="1" ${item.recipe ? 'disabled title="Record another recipe for another bottle"' : ''}>+</button><button class="remove-item" data-remove="${item.id}" aria-label="Remove ${escapeHtml(item.name)}">×</button></div>${item.recipe ? `<span class="recipe-note">${item.recipe.map(line => `${escapeHtml(line.name)} ${line.ml.toFixed(2)}ml`).join(' · ')}${item.packMaterialId ? ` · ${(item.packQty || 1)} × ${escapeHtml(findMaterial(item.packMaterialId)?.name || 'packaging')}` : ''}${(item.extras || []).map(extra => ` · ${extra.qty} × ${escapeHtml(extra.name)}`).join('')}</span>` : ''}</div><div class="cart-price">${currency(item.price * item.quantity)}</div></div>`).join('');
  }
  renderTotals();
  persistCart();
  renderProducts();
...
function renderRecipeExtras() {
  ...
  $('#recipeExtrasWrap').classList.toggle('hidden', !choices.length);
  $('#recipeExtras').innerHTML = choices.map(material => `<label class="recipe-row"><span><input type="checkbox" data-extra="${material.id}" /><strong>${escapeHtml(material.name)}</strong><small>${material.type} · ${fmtQty(availableMl(material), 'pcs')} available</small></span><input class="ml-input" type="number" min="1" max="99" step="1" value="1" data-extra-qty="${material.id}" aria-label="${escapeHtml(material.name)} pieces" /></label>`).join('');
}
...
function openRecipe(product) {
  ...
  $('#recipeHelp').textContent = `Record every attar and raw material used. The total must be exactly ${product.recipeMl}ml.${pack ? ` ${(product.packQty || 1)} × ${pack.name} is taken from stock automatically.` : ''}`;
  $('#recipeSearch').value = '';
...
function renderRecipeOptions() {
  ...
  const visible = query ? matching : pool.slice(0, 8);
  const visibleMaterials = [...new Map([...visible, ...pool.filter(material => selectedIds.has(material.id))].map(material => [material.id, material])).values()];
  $('#recipeFilterStatus').textContent = query ? `${matching.length} material${matching.length === 1 ? '' : 's'} found` : `Showing ${visibleMaterials.length} of ${pool.length} materials · search to find another`;
  $('#recipeOptions').innerHTML = visibleMaterials.length ? visibleMaterials.map(material => `<label class="recipe-row"><span><input type="checkbox" data-material="${material.id}" ${selectedIds.has(material.id) ? 'checked' : ''} /><strong>${escapeHtml(material.name)}</strong><small>${material.type} · ${availableMl(material).toFixed(2)}ml available</small></span><input class="ml-input" type="number" min="0" step="0.01" value="${quantities.get(material.id) ?? '0.00'}" data-ml="${material.id}" aria-label="${escapeHtml(material.name)} quantity in ml" /></label>`).join('') : '<div class="recipe-empty">No materials match that search.</div>';
}
...
function saveRecipe() {
  ...
  db.cart.push({ ...recipeProduct, price: priceFor(recipeProduct, billingType()), id: `${recipeProduct.id}-${Date.now()}`, productId: recipeProduct.id, name, type: recipeProduct.name, quantity: 1, recipe, extras });
