...
function createSale({ body, user, outlet }) {
  ...
  return tx(() => {
    ...
    for (const line of requested) {
      ...
      let pack = null;
      if (product.pack_material_id && line.includePack !== false) {
...
const bulkImport = require('./bulk-import')({ db, saveProduct, saveMaterial, createCustomer, HttpError });
const routes = [
  ['POST', '/api/import/products', 'admin', bulkImport('products')],
  ['POST', '/api/import/materials', 'admin', bulkImport('materials')],
